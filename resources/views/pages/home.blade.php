    ...
    <div class="container-xxl">
        ...
        <div class="hp-top-header mb-3">
            ...
            <div class="hp-live-stream-bar" id="hp-live-stream-bar">
                ...
                @php
                    /* Always-animated premium transaction pool (high-value samples rotate
                       with recent real orders so the stream never looks empty/slow) */
                    $premiumPool = [
                        ['ticker-cat-cashout','Cashout','Paysafecard €500 Cashout',  62250, 'USDT',  '3 min ago',  '01***7712'],
                        ['ticker-cat-topup',  'Top-Up','Free Fire 2260 Diamonds',      1990,  'bKash', '4 min ago',  '016***8823'],
                        ['ticker-cat-cashout','Cashout','Transcash €350 Voucher',    39400, 'Nagad', '6 min ago',  '017***3345'],
                        ['ticker-cat-buy',    'Buy',   'Apple Gift Card $100',        12900, 'bKash', '8 min ago',  '019***5614'],
                        ['ticker-cat-topup',  'Top-Up','PUBG Mobile 1950 UC',         2680,  'bKash', '9 min ago',  '015***9920'],
                        ['ticker-cat-cashout','Cashout','Apple Card $250 Cashout',   27450, 'USDT',  '11 min ago', '018***1189'],
                        ['ticker-cat-subs',   'Sub',    'Netflix Premium Annual',      3360,  'bKash', '13 min ago', '013***4556'],
                        ['ticker-cat-cashout','Cashout','Steam Wallet €200 Cashout',  27040, 'Nagad', '17 min ago', '016***7702'],
                        ['ticker-cat-buy',    'Buy',   'Razer Gold 100 USD PIN',      13000, 'Rocket','21 min ago', '014***8890'],
                        ['ticker-cat-cashout','Cashout','Neosurf €100 Voucher',       11265, 'Nagad', '25 min ago', '017***0983'],
                    ];
                    $streamItems = collect();
                    if (isset($liveTransactions)) {
                        foreach ($liveTransactions->take(4) as $tx) {
                            $streamItems->push([
                                $tx->category_class, $tx->category_label, $tx->product_title,
                                $tx->amount_bdt, $tx->payment_method, $tx->time_text, $tx->customer_masked,
                            ]);
                        }
                    }
                    collect($premiumPool)->shuffle()->each(function ($p) use (&$streamItems) {
                        if ($streamItems->count() < 9) $streamItems->push($p);
                    });
                    $streamItems = $streamItems->shuffle()->values();
                @endphp
                    ...
                    <div class="hp-live-vertical-track" id="hp-live-track">
                        @foreach($streamItems as $item)
                            <div class="hp-live-vt-item">
                                <span class="hp-live-vt-badge {{ $item[0] }}">{{ $item[1] }}</span>
                                <span class="hp-live-vt-title">{{ $item[2] }}</span>
                                <span class="hp-live-vt-amount">৳{{ number_format($item[3]) }}</span>
                                <span class="hp-live-vt-method">via {{ $item[4] }}</span>
                                <span class="hp-live-vt-time">{{ $item[5] }}</span>
                                <span class="hp-live-vt-user">({{ $item[6] }})</span>
                            </div>
                        @endforeach
...
@php
    ...
    /* Official brand logos are resolved centrally via \App\Support\BrandLogos::logo($slug) */
@endphp
...
<section class="hp-calc-section" aria-label="Gift Card Rate Calculator and Quick Tools Hub">
    <div class="container-xxl">
        ...
        <!-- Section Header (Redesigned: badge + gradient title + metric chips) -->
        <div class="hp-dual-header mb-3">
            <div class="hp-dual-head-badge">
                <i data-lucide="sparkles" class="icon"></i>
                <span>Rate &amp; Quick Tools Hub</span>
                <span class="hp-dual-head-dot" aria-hidden="true"></span>
            </div>
            <div class="hp-dual-head-row">
                <h2 class="hp-section-title m-0" id="hp-dual-heading">
                    Instant <span class="hp-grad-text">Rate Calculator</span> &amp; Quick Tools
                </h2>
                <div class="hp-dual-head-chips" aria-label="Tool highlights">
                    <span class="hp-dual-chip"><i data-lucide="badge-percent" class="icon"></i>84–90% Payout</span>
                    <span class="hp-dual-chip hp-dual-chip-live"><span class="hp-live-dot"></span>Live Rates</span>
                </div>
            </div>
            <p class="hp-dual-head-sub bn">
                ক্যালকুলেটর, প্রাইজ চেক ও USDT কনভার্টার — এক সেকশনে দুই টুল, মোবাইলে সোয়াইপ করুন
            </p>
        </div>
...
<section class="hp-section animate-in" aria-labelledby="hp-trending-heading">
    <div class="container-xxl">
        <div class="d-flex align-items-end justify-content-between gap-2 mb-3">
            ...
            <div class="tr-rail-actions">
                <a href="{{ route('subscriptions') }}" class="hp-view-all">
                    <span>View All</span>
                    <i data-lucide="arrow-right" class="icon" style="width:14px;height:14px;"></i>
                </a>
                <div class="tr-arrows" aria-label="Scroll trending products">
                    <button type="button" class="tr-arrow" id="tr-rail-prev" aria-label="Previous products"><i data-lucide="chevron-left" class="icon"></i></button>
                    <button type="button" class="tr-arrow" id="tr-rail-next" aria-label="Next products"><i data-lucide="chevron-right" class="icon"></i></button>
                </div>
            </div>
        </div>

        <div class="row row-cols-2 row-cols-md-3 row-cols-lg-4 g-2 g-md-3 tr-rail" id="tr-rail">
                    ...
                    <a href="{{ $tpRoute }}" class="tr-card h-100" title="{{ $tp->title }}" style="--tr-brand: {{ $tp->brand_color }};">
                        ...
                        <div class="tr-card-top">
                            <span class="tr-icon-tile">
                                @php $tpLogo = \App\Support\BrandLogos::logo($tp->slug); @endphp
                                @if($tpLogo)
                                    <img src="{{ asset('images/brands/' . $tpLogo) }}" class="hp-brand-logo" width="30" height="30" loading="lazy" decoding="async" alt="{{ $tp->title }} brand logo">
                                @else
                                    <i data-lucide="{{ $tpIcon }}" class="icon"></i>
                                @endif
                            </span>
                            <div class="tr-card-headside">
                                {{-- Single tag per card (duplicate second chip removed as requested) --}}
                                @if($tp->tag_badge)
                        ...
                        <div class="hp-mini-thumb hp-thumb-square">
                            ...
                            @else
                                <div class="hp-mini-thumb-inner" style="background: radial-gradient(circle, {{ $game->brand_color }}22 0%, transparent 80%);">
                                    @php $gameLogo = \App\Support\BrandLogos::logo($game->slug); @endphp
                                    @if($gameLogo)
                                        <img src="{{ asset('images/brands/' . $gameLogo) }}" class="hp-brand-logo" width="46" height="46" loading="lazy" decoding="async" alt="{{ $game->title }} logo">
                                    @else
    ...
    <div class="container-xxl">
        ...
        <div class="row row-cols-2 row-cols-md-3 row-cols-lg-4 g-2 g-md-3">
            @foreach($subscriptions as $sub)
                <div class="col">
                    @include('partials.service-mini-card', ['service' => $sub, 'route' => 'subscriptions.show', 'ctaLabel' => 'Get Account', 'fallbackIcon' => 'crown'])
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="container-xxl">
        ...
        <div class="row row-cols-2 row-cols-md-3 row-cols-lg-4 g-2 g-md-3">
            @foreach($giftCardBuys as $card)
                <div class="col">
                    @include('partials.service-mini-card', ['service' => $card, 'route' => 'giftcards.buy.show', 'ctaLabel' => 'Buy Code', 'fallbackIcon' => 'gift'])
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="container-xxl">
        ...
        <div class="row row-cols-2 row-cols-md-3 row-cols-lg-4 g-2 g-md-3">
            @foreach($softwareProducts as $soft)
                <div class="col">
                    @include('partials.service-mini-card', ['service' => $soft, 'route' => 'digital.products.show', 'ctaLabel' => 'Get License'])
                </div>
            @endforeach
        </div>
    </div>
...
<section class="hp-section animate-in" aria-labelledby="hp-how-heading">
    <div class="container-xxl">
        ...
        <div class="row g-3">
            ...
            <div class="col-12 col-md-4">
                <div class="hp-how-card">
                    <div class="hp-how-step" style="background: rgba(31, 163, 126, 0.15); border-color: rgba(31, 163, 126, 0.35); color: var(--accent-amber);" aria-hidden="true">2</div>
...
<section class="hp-section animate-in" aria-label="Platform guarantees">
    <div class="container-xxl">
        <div class="row row-cols-2 row-cols-lg-4 g-2 g-md-3">
            ...
            <div class="col">
                <div class="hp-trust-card h-100">
                    <div class="hp-trust-icon" style="color: var(--accent-amber); background: rgba(31, 163, 126, 0.12);">
