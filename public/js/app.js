...
(function () {
  ...
  // Currency symbol mapping (avoids hardcoded $ for EUR/GBP cards)
  ...
  function initExchangeCalculator() {
    ...
    function calculate() {
      ...
      const activeSymbol = CURRENCY_SYMBOLS[currency] || '$';
      ...
      if (currentMode === 'sell') {
        ...
        if (calcPayoutDisplay) {
          calcPayoutDisplay.innerHTML = `৳ ${payoutBDT.toLocaleString()} <span style="font-size:0.75rem; font-weight:600; color:var(--text-dim);">(${activeSymbol}${payoutUSD.toFixed(2)})</span>`;
        }
        if (calcActionBtn) {
        ...
        if (calcPayoutDisplay) {
          calcPayoutDisplay.innerHTML = `৳ ${costBDT.toLocaleString()} <span style="font-size:0.75rem; font-weight:600; color:var(--text-dim);">(${activeSymbol}${costUSD.toFixed(2)})</span>`;
        }
        if (calcActionBtn) {
          // Avoid "Paysafecard Voucher Voucher" when brand name already ends with "Voucher"
          let cleanBrandName = brand.name;
          if (cleanBrandName.toLowerCase().endsWith(' voucher')) {
            cleanBrandName = cleanBrandName.substring(0, cleanBrandName.length - 8).trim();
          }
          calcActionBtn.innerHTML = `<span>Buy ${cleanBrandName} Voucher</span> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>`;
  ...
  // --- Trending Products rail: desktop prev/next arrows ---
  function initTrendingRail() {
    const rail = document.getElementById('tr-rail');
    const prev = document.getElementById('tr-rail-prev');
    const next = document.getElementById('tr-rail-next');
    if (!rail || !prev || !next) return;

    function cardStep() {
      const col = rail.querySelector(':scope > .col');
      return col ? col.offsetWidth + 10 : 220;
    }

    prev.addEventListener('click', () => {
      rail.scrollBy({ left: -(cardStep() * 2), behavior: 'smooth' });
    });
