...
<body class="has-rp-rail">
        ...
        <div class="container">
            <div class="navbar-wrapper">
                ...
                <div class="brand-group">
                    ...
                    <!-- Currency Selector Pill (Code Only) -->
                    <button type="button" class="currency-selector-pill" id="currency-selector-btn" aria-label="Select Currency" title="Select Currency">
                        <span class="curr-flag d-none" id="active-curr-flag">🇧🇩</span>
                        <span class="curr-code" id="active-curr-code">BDT</span>
                        <span class="curr-sym d-none" id="active-curr-sym">৳</span>
                        <i data-lucide="chevron-down" class="icon curr-caret"></i>
                    </button>
                </div>
                ...
                <div class="nav-actions">
                    ...
                    <!-- Theme Toggle (Desktop Only - Mobile in Drawer) -->
                    <button type="button" class="btn-icon d-none d-lg-inline-flex" id="theme-toggle-btn" title="Toggle Dark / Light Theme" aria-label="Toggle Dark / Light Theme">
                        <i data-lucide="moon" class="icon theme-icon-moon"></i>
                        <i data-lucide="sun" class="icon theme-icon-sun"></i>
                    </button>

                    @guest
                        <!-- Login Link -->
                        <a href="{{ route('login') }}" class="btn-icon header-admin-link" title="Login / Sign Up" aria-label="Login or Sign Up">
                            <i data-lucide="user" class="icon"></i>
                        </a>
                    @else
                        @if(auth()->user()->is_admin)
                            <!-- Admin Link -->
                            <a href="{{ route('admin.dashboard') }}" class="btn-icon header-admin-link" title="Admin Portal" aria-label="Admin Portal" style="color: var(--vb-orange); border-color: rgba(255, 140, 0, 0.4); background: rgba(255, 140, 0, 0.1);">
                                <i data-lucide="shield" class="icon"></i>
                            </a>
                        @endif
                        <!-- Logout -->
                        <form action="{{ route('logout') }}" method="POST" class="m-0 d-inline-flex">
                            @csrf
                            <button type="submit" class="btn-icon header-admin-link" title="Logout ({{ auth()->user()->name }})" aria-label="Logout">
    ...
    <div id="currency-modal" class="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="currency-modal-title">
        <div class="modal-box currency-modal-box">
            <button type="button" id="currency-close-btn" class="modal-close-btn" aria-label="Close">
                <i data-lucide="x" class="icon"></i>
            </button>
            <div style="margin-bottom: 1rem;">
                <div id="currency-modal-title" style="font-size: 1.15rem; font-weight: 800; margin-bottom: 0.2rem; font-family: var(--font-heading);">Select Country & Currency</div>
                <div style="font-size: 0.8rem; color: var(--text-dim);">Supported payment gateways and pricing adapt instantly to your selected region</div>
            </div>
